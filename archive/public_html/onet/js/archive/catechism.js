// function generateLinks(data, subtopicName) {
//     let subtopicData = data.filter(item => item.subtopic === subtopicName);
//     let links = '';
//     subtopicData.forEach(item => {
//         item.formats.forEach(format => {
//             let filePath = item.path + item.file + '.' + format;
        ...
        let sections = [...sectionNames].map(name => {
            ...
            let topicDivs = topics.map(subtopic =>
                `<div class="row">
                    ...
                    <div class="six columns" style="text-align:center;">${generateLinks(data, subtopic)}</div>
                </div>`
            ).join('');
