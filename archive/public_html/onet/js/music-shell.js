document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!fileName) {
        const pathMatch = window.location.pathname.match(/\/music\/(.+\.html)$/);
        if (pathMatch) {
    ...
    if (fileName) {
        ...
        fetch(`/music_fetcher.php?file=${encodeURIComponent(fileName)}`)
            ...
            .then(html => {
                ...
                document.getElementById('music-placeholder').innerHTML = html;
                
                // Update page title if found
                updatePageTitle();
            })
            .catch(error => {
                document.getElementById('music-placeholder').innerHTML = 
...
function cleanLegacyHTML(html) {
    ...
    html = html.replace(/href="([^"]*\.(mp3|pdf|doc|docx|rtf))"/gi, function(match, url) {
        ...
        return 'href="https://www.orthodox.net/music/' + url + '"';
    });
    ...
    html = html.replace(/src="([^"]+)"/gi, function(match, url) {
        ...
        if (url.match(/^[^\/]+\.(gif|jpg|jpeg|png|webp)$/i)) {
            return 'src="https://www.orthodox.net/music/' + url + '"';
        }
        return match;
    });
...
function updatePageTitle() {
    const h3 = document.querySelector('#music-placeholder h3');
    if (h3) {
