document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!fileName) {
        const pathMatch = window.location.pathname.match(/\/nativity\/(.+\.html)$/);
        if (pathMatch) {
    ...
    if (fileName) {
        ...
        fetch(`/nativity_fetcher.php?file=${encodeURIComponent(fileName)}`)
            ...
            .then(html => {
                ...
                document.getElementById('nativity-placeholder').innerHTML = html;
                
                // Update page title if found
                updatePageTitle();
            })
            .catch(error => {
                document.getElementById('nativity-placeholder').innerHTML = 
...
function cleanLegacyHTML(html) {
    ...
    html = html.replace(/href="([^"]*\.(mp3|pdf|doc|docx|rtf))"/gi, 
        'href="https://www.orthodox.net/nativity/$1"');
    ...
    html = html.replace(/href="\.\.\/menaion-december\//gi, 'href="/menaion-december/');
    html = html.replace(/href="\.\.\/services\//gi, 'href="/services/');
    html = html.replace(/href="\.\.\/trebnic\//gi, 'href="/trebnic/');
    html = html.replace(/href="\.\.\/troparia\//gi, 'href="/troparia/');
    ...
    html = html.replace(/src="([^"]+)"/gi, function(match, url) {
        ...
        if (url.match(/^[^\/]+\.(gif|jpg|jpeg|png|webp)$/i)) {
            return 'src="https://www.orthodox.net/nativity/' + url + '"';
        }
        return match;
    });
...
function updatePageTitle() {
    const h3 = document.querySelector('#nativity-placeholder h3');
    if (h3) {
