document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!fileName) {
        const pathMatch = window.location.pathname.match(/\/poetry\/(.+\.html)$/);
        if (pathMatch) {
    ...
    if (fileName) {
        ...
        fetch(`/poetry_fetcher.php?file=${encodeURIComponent(fileName)}`)
            ...
            .then(html => {
                ...
                document.getElementById('poetry-placeholder').innerHTML = html;
                
                // Update page title if found
                updatePageTitle();
            })
            .catch(error => {
                document.getElementById('poetry-placeholder').innerHTML = 
...
function cleanLegacyHTML(html) {
    ...
    html = html.replace(/href="([^"]*\.(mp3|pdf|doc|docx|rtf))"/gi, function(match, url) {
        if (!url.startsWith('http://') && !url.startsWith('https://') && !url.startsWith('/')) {
            return 'href="https://www.orthodox.net/poetry/' + url + '"';
        }
        return match;
    });
    ...
    html = html.replace(/src="([^"]+)"/gi, function(match, url) {
        ...
        if (url.match(/^[^\/]+\.(gif|jpg|jpeg|png|webp)$/i)) {
            return 'src="https://www.orthodox.net/poetry/' + url + '"';
        }
        return match;
    });
...
function updatePageTitle() {
    const h3 = document.querySelector('#poetry-placeholder h3');
    if (h3) {
