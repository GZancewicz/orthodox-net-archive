document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!fileName) {
        const pathMatch = window.location.pathname.match(/\/western-saints\/(.+\.html)$/);
        if (pathMatch) {
    ...
    if (fileName) {
        ...
        fetch(`/western-saints_fetcher.php?file=${encodeURIComponent(fileName)}`)
            ...
            .then(html => {
                ...
                document.getElementById('western-saints-placeholder').innerHTML = html;
                
                // Update page title if found
                updatePageTitle();
            })
            .catch(error => {
                document.getElementById('western-saints-placeholder').innerHTML = 
...
function cleanLegacyHTML(html) {
    ...
    html = html.replace(/href="([^"]*\.(mp3|pdf|doc|docx|rtf))"/gi, function(match, url) {
        if (!url.startsWith('http://') && !url.startsWith('https://') && !url.startsWith('/')) {
            return 'href="https://www.orthodox.net/western-saints/' + url + '"';
        }
        return match;
    });
    ...
    html = html.replace(/src="([^"]+)"/gi, function(match, url) {
        ...
        if (url.match(/^[^\/]+\.(gif|jpg|jpeg|png|webp)$/i)) {
            return 'src="https://www.orthodox.net/western-saints/' + url + '"';
        }
        return match;
    });
...
function updatePageTitle() {
    const h3 = document.querySelector('#western-saints-placeholder h3');
    if (h3) {
